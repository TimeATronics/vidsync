...
/**
 * Fetch stream sources for a movie or TV episode via direct HTTP calls to the
 * rivestream backendfetch API. No browser automation needed.
 */
export async function getSources(
  ...
  const sources = await getRiveSources(type, id, season, episode);
  return sources.map(s => ({ label: s.label, url: s.url, format: s.format }));
}
